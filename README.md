...
1. **Clone** the repository and **navigate** to the Terraform directory:
   ...
   ```bash
   git clone https://github.com/confluentinc/demo-cross-cloud-replication.git
   cd demo-cross-cloud-replication/terraform
   ```


2. In `terraform` directory,  create a `terraform.tfvars` file to store the Confluent Cloud API keys required by Terraform. Replace the placeholders below with your own keys.
...
3. **Initialize and deploy the Terraform infrastructure:**
...
4. **Load environment variables** (generated by Terraform) so you can copy-paste commands directly:
...
Since the clusters are private, you’ll need to update your local hosts file so your browser and CLI can reach Confluent through the NGINX proxies. Once updated, you can run all commands directly from your local machine.

To update your hosts file (choose your OS), copy and run the combined command from the Terraform outputs:

- **macOS/Linux:** `update_hosts_macos_linux_all`  
- **Windows:** `update_hosts_windows_all`

Alternatively, you can use **Step 1** in the `replication_commands.txt` file, which includes commands for both OS options.

> **Note:** `replication_commands.txt` is generated by Terraform in the `terraform/` directory and contains OS-specific commands for all setup steps.
...
Create sample data in the source cluster. For this, we’ll use the DataGen Source connector. 
...
> **Note:** Terraform generates OS‑specific commands in `replication_commands.txt` (macOS/Linux and Windows CMD). Use these and run the steps locally:
...
> **Note:** You can copy the OS‑specific block from `replication_commands.txt` (generated by Terraform).
   ...
   confluent kafka link create cross-cloud-link \
     ...
     --source-api-key $SOURCE_KAFKA_API_KEY \
     --source-api-secret $SOURCE_KAFKA_API_SECRET \
   ...
   confluent kafka link create cross-cloud-link `
     --cluster $env:DEST_CLUSTER_ID `
     --source-cluster $env:SOURCE_CLUSTER_ID `
     --source-bootstrap-server $env:SOURCE_BOOTSTRAP `
     --source-api-key $env:SOURCE_KAFKA_API_KEY `
     --source-api-secret $env:SOURCE_KAFKA_API_SECRET `
     --config link-config.properties
   ```
   </details>
...
```bash
confluent connect cluster delete <DATAGEN_CONNECTOR_ID> --cluster $SOURCE_CLUSTER_ID --environment $SOURCE_ENV_ID --force
```
